...
auto VCFReader(CoordSystem cs)(TabixIndexedFile tbxFile, Interval!cs coords, int MAX_UNPACK = BCF_UN_ALL)
{
    return VCFReaderImpl!(cs,true)(tbxFile, coords, MAX_UNPACK);
}
...
struct VCFReaderImpl(CoordSystem cs, bool isTabixFile)
{
    ...
    static if(isTabixFile){
        ...
        this(TabixIndexedFile tbxFile, Interval!cs coords, int MAX_UNPACK = BCF_UN_ALL)
        {
            this.tbx = tbxFile;
            this.tbxRange = initializeTabixRange(coords);
            this.tbxRange.empty;
        ...
        auto initializeTabixRange(Interval!cs coords)
        {
            return this.tbx.region(coords);
        }
    }else{
...
debug(dhtslib_unittest) unittest
{
    ...
    auto vcf = VCFReader(tbx, reg);
    assert(vcf.count == 3);
    vcf = VCFReader(tbx, reg);
    vcf.empty;
    VCFRecord rec = vcf.front;
    assert(rec.chrom == "1");
